...
<template>
  <div class="taily">
    <searchForm :formOptions="formOptions" @onSearch="handleSearch">
      <el-button slot="other" icon="el-icon-s-ticket" size="mini" type="primary" @click="onWithdraw">提现</el-button>
    </searchForm>
              ...
              <div class="btnBox">
                <div style="margin-bottom: 20px;">
                  ...
                  <el-button
                    ...
                    type="text"
                    icon="el-icon-check"
                    @click="onAcceptOrRefuse(scope.row, 1)"
                    :disabled="scope.row.status !== 'WaitAccept'"
                    >接受</el-button
                  >
                  <el-button
                    size="mini"
                    type="text"
                    icon="el-icon-close"
                    @click="onAcceptOrRefuse(scope.row, 0)"
                    :disabled="scope.row.status !== 'WaitAccept'"
                    >拒绝</el-button
                  >
                  <el-button
                ...
                <div>
                  <el-button
                    ...
                    type="text"
                    icon="el-icon-sort"
                    @click="onConfirmTransfer(scope.row)"
                    :disabled="scope.row.status !== 'WaitPayment'"
                    >确认转账</el-button
                  >
                  <el-button
                    size="mini"
...
import {
  getOrderList, // 所有订单列表API
  disposeOrder, // 处理订单 接受or拒绝API
  confirmTransfer, // 确认转账API
  operaMakeLoans, // 放款API
  getOrderStatus, // 订单状态列表API
  coinfirmAddress // 确认地址API
  ...
  methods: {
    ...
    onAcceptOrRefuse(row, type) {
      ...
      this.msgConfirmFunc({ txt: `确认${txt}该订单` }, () => {
        const params = { orderId: row.id, result: type }
      ...
      getOrderList(params).then(res => {
        ...
        this.tableList = list.map(item => {
          if (item.uploadRecord) {
            // 图片有没有域名都去掉域名再统一加上域名
            const imgUrl = tools.domainFilter(item.uploadRecord) // 去掉域名的图片地址
